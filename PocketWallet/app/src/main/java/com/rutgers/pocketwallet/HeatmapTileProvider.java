...
public class HeatmapTileProvider implements TileProvider {
    ...
    /**
     * Colors for defaultlocation Gradient.
    ...
    /**
     * Starting fractions for defaultlocation Gradient.
     ...
     */
    public static final HM_Gradient DEFAULT_HM_GRADIENT = new HM_Gradient(DEFAULT_GRADIENT_COLORS, DEFAULT_GRADIENT_START_POINTS);
     ...
     */
    private PointQuadTree<HM_WeightedLatLng> mTree;

    /**
     * Collection of all the data.
     */
    private Collection<HM_WeightedLatLng> mData;
    ...
    /**
     * HM_Gradient of the color map
     */
    private HM_Gradient mGradient;
    ...
    public static class Builder {
        ...
        private Collection<HM_WeightedLatLng> data;

        // Optional, initialised to defaultlocation values
        private int radius = DEFAULT_RADIUS;
        private HM_Gradient HM_Gradient = DEFAULT_HM_GRADIENT;
         ...
         * @return updated builder object
         */
        public Builder weightedData(Collection<HM_WeightedLatLng> val) {
            this.data = val;
         ...
         * @return updated builder object
         */
        public Builder gradient(HM_Gradient val) {
            HM_Gradient = val;
            return this;
        }
     ...
     *             changing the radius.
     */
    public void setWeightedData(Collection<HM_WeightedLatLng> data) {
        // Change point set
        mData = data;
        ...
        mTree = new PointQuadTree<HM_WeightedLatLng>(mBounds);

        // Add points to quad tree
        for (HM_WeightedLatLng l : mData) {
            mTree.add(l);
        }
     ...
     * @return Data, in HM_WeightedLatLng form
     */
    private static Collection<HM_WeightedLatLng> wrapData(Collection<LatLng> data) {
        // Use an ArrayList as it is a nice collection
        ArrayList<HM_WeightedLatLng> weightedData = new ArrayList<HM_WeightedLatLng>();

        for (LatLng l : data) {
            weightedData.add(new HM_WeightedLatLng(l));
        }
    ...
    public Tile getTile(int x, int y, int zoom) {
        ...
        double xOffset = 0;
        Collection<HM_WeightedLatLng> wrappedPoints = new ArrayList<HM_WeightedLatLng>();
        if (minX < 0) {
        ...
        Collection<HM_WeightedLatLng> points = mTree.search(tileBounds);
        ...
        for (HM_WeightedLatLng w : points) {
            Point p = w.getPoint();
        ...
        for (HM_WeightedLatLng w : wrappedPoints) {
            Point p = w.getPoint();
     ...
     * User should clear overlay's tile cache (using clearTileCache()) after calling this.
     *
     * @param HM_Gradient Gradient to set
     */
    public void setGradient(HM_Gradient HM_Gradient) {
        mGradient = HM_Gradient;
        mColorMap = HM_Gradient.generateColorMap(mOpacity);
    }
     ...
     *
     * @param points Collection of HM_WeightedLatLng to calculate bounds for
     * @return Bounds that enclose the listed HM_WeightedLatLng points
     */
    static Bounds getBounds(Collection<HM_WeightedLatLng> points) {

        // Use an iterator, need to access any one point of the collection for starting bounds
        Iterator<HM_WeightedLatLng> iter = points.iterator();

        HM_WeightedLatLng first = iter.next();
     ...
     * @param max      Maximum intensity value: maps to 100% on HM_Gradient
     ...
     */
    static double getMaxValue(Collection<HM_WeightedLatLng> points, Bounds bounds, int radius,
                              int screenDim) {
        ...
        double x, y;
        double max = 0;
        for (HM_WeightedLatLng l : points) {
            x = l.getPoint().x;
            y = l.getPoint().y;
